...
interface WorkoutDao {
    ...
    /**
     * A LEFT JOIN rather than two flows combined in Kotlin: one query, one
     * invalidation, one emission — and the pair stays atomic.
     */
    @Query(
        """
        SELECT a.id            AS id,
    ...
    /** Touches only `workout_assignments` — never `completion_overrides`. */
    @Transaction
