...
| 0.3 | Design system exists: tokens for colour, type, shape and spacing, and the app renders through the theme | Placeholder screen uses **no** literal values |
| 0.4 | Test harness runs: JVM test deps wired, `MainDispatcherRule` present, fixture on the test classpath | A real (non-example) JVM test executes |
| 0.5 | A single command runs build + unit tests + lint and exits non-zero on any failure | Deliberately break a test; confirm non-zero exit |
| 0.6 | `CLAUDE.md` records build/test/lint commands, conventions and healthy output | ✅ Done |
...
**Rung 0.3 is scaffolding, not design work.** The tokens exist with placeholder
values; the design fills them in later. What matters is that by the time Intent
04 starts, there is exactly one file to edit — not thirty composables holding
inline hex.

**Rung 0.4 is not implied by 0.5.** A gate command can exit zero while running
...
on the test classpath — is its own rung, and rung 0.5 then proves it works.
